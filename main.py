...
BOT_TOKEN = "5087617928:AAGfg8apWEnFeCRWUqgl-1M99COE_DsB7dc"
PAYMENT_CHANNEL = "@Paymentofbots" #add payment channel here including the '@' sign
OWNER_ID = 1194007250 #write owner's user id here.. get it from @MissRose_Bot by /id
CHANNELS = ["@EARNINGMAHAL","@Software_looter_s","@LOOTERKARAN"] #add channels to be checked here in the format - ["Channel 1", "Channel 2"] 
...
def send_text(message):
   try:
    ...
    if message.text == "⚙️ Set Wallet":
        ...
        keyboard = telebot.types.ReplyKeyboardMarkup(True)
        keyboard.row('🚫 Cancel')
        send = bot.send_message(message.chat.id, "_⚠️Send your Paytm Wallet Address._",
